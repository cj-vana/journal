...
export default function UserManagement({ currentUserId }: { currentUserId: string }) {
  ...
  async function handleDeleteInvite(id: string) {
    // Revoke by updating maxUses to current useCount
    const invite = invites.find((i) => i.id === id)
    if (!invite) return
    // Just remove from UI - we don't have a delete endpoint for invites,
    // but we can re-fetch after the user sees the removal
