...
export function GrowthPageClient({ initialRecords, userId, userRole }: Props) {
  ...
  const heightData = useMemo(
    () => records.filter((r) => r.heightCm !== null).map((r) => ({ date: r.date, value: r.heightCm as number })),
    [records]
  )

  const weightData = useMemo(
    () => records.filter((r) => r.weightKg !== null).map((r) => ({ date: r.date, value: r.weightKg as number })),
    [records]
  )

  const headData = useMemo(
    () => records.filter((r) => r.headCm !== null).map((r) => ({ date: r.date, value: r.headCm as number })),
    [records]
  )
                      ...
                      <td className="px-4 py-3">
                        {canDelete && (
                          <button
                            ...
                            className="text-warm-600 hover:text-red-600 transition-colors"
                            title="Delete"
                          >
