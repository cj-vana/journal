...
function embedImage(mediaPath: string, mimeType: string): string {
  try {
...
export function renderEntryPage(entry: EntryForPdf): string {
  const entryDate =
  ...
  const images = entry.media
    ...
    .map((m) => {
      const dataUri = embedImage(m.path, m.mimeType)
      if (!dataUri) return ''
      return `
        <div style="margin: 16px 0; text-align: center;">
          <img src="${dataUri}" style="max-width: 100%; max-height: 400px; border-radius: 8px; border: 1px solid #e8ddd4;" />
          ${m.caption ? `<p style="font-family: 'Inter', sans-serif; font-size: 12px; color: #a08b7a; margin-top: 4px;">${m.caption}</p>` : ''}
        </div>
      `
    })
    .join('')
  ...
  const contentHtml = entry.contentHtml || `<p>${entry.content}</p>`
