...
export function renderChapter(yearLabel: string, stats: ChapterStats, colors: AccentColors): string {
  const milestoneList = stats.milestoneHighlights
    .map((m) => `<li style="margin-bottom: 4px; color: #8b6f5c;">${escapeHtml(m)}</li>`)
    .join('')
